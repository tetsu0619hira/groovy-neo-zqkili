...
- 公開先で住所検索のピンと住所「神城22742-1」を目視確認。店名検索時と同じ位置（神城駅北側・国道148号付近）に表示。住所カードには評価・件数を表示しない。
...
## 完成時の検証（2026-09-19）
- 公開URL：https://tetsu0619hira.github.io/groovy-neo-zqkili/ を実際に開いて確認。
- publicリポジトリ：https://github.com/tetsu0619hira/groovy-neo-zqkili 。GitHub Pagesはmainブランチ・ルート、ビルド成功。
- 375×812pxで営業時間・電話CTA・固定バーを確認。横はみ出しなし。固定バー高さ69px。ページ最下部のデモ表記・著作権表示が隠れないことを確認。
- モバイルメニュー開閉、ナビ選択後の閉鎖、ページ内移動を実操作で確認。PCでは固定バー非表示。
- 全3画像の読み込み成功・重ねたイメージ写真キャプションを確認。各画像200KB以内。
- 電話10リンクはすべてtel:0261850990、電話リンクの外部リンク記号なし。外部リンクの新規タブ・noopenerを確認。
- noindex・lang=ja・指定title・デモ表記を確認。フォームなし、Instagram埋め込みなし。
- 担当者／スタイリストの個人名、併設サロンの予約ページURL、類似店名、口コミ写真、料金の具体額、評価の数字はindex.htmlに掲載なし。地図も評価なしの住所表示へ変更。
- 未確認欄は25か所・19種類（料金／所要時間を各4メニューに設置）。CHECKLIST.mdに重複をまとめて一覧化。
- コントラスト比：白／テラコッタ5.44:1、本文／オフホワイト11.88:1、補助文字／オフホワイト5.56:1、確認欄8.76:1。
- 実際の電話発信・Instagramへの連絡は行っていない。未確認の店舗情報は店主確認が必要。
